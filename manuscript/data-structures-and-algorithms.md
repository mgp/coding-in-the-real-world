...
Classes on data structures and algorithms can be overwhelming. There are so many things to learn! Constructing a minimum spanning tree or computing a convex hull is only the tip of the iceberg. But unless your work is highly specialized, you will not need many of these data structures and algorithms. Instead you will repeatedly use only a small handful of them. Now you may think that if you only need to know a handful of data structures and algorithms, then you can get by without knowing any. But that is patently false. Understanding and employing the fundamental data structures and algorithms will not only make your code more efficient, as measured by execution time and memory usage, but more correct. These are the hallmarks of any well-designed program.
